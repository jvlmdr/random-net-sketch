...
In a toy experiment, as the depth increases, $\E[y_{i}^{\A} y_{i}^{\B}]$ approaches $\V[y_{i}^{\A}] = \E[(y_{i}^{\A})^2]$ (using $\V[W_{i}] = 2 / m_{i}$).
...
However, experimentally, the variance of $y_{L+1}$ is significantly larger.
